...
async function watch(params: { filter: string }) {
  ...
  const rebuild_debounced = debounce(() => signal.resolve(true), 16);
...
async function build(params: {
  ...
  await update_file("out/www/index.html", html_ugly(PostList({ posts })));
  await update_file("out/www/feed.xml", feed_xml(posts));
  for (const post of posts) {
    await update_file(`out/www${post.path}`, html_ugly(Post({ post })));
  }
  ...
  for (const page of pages) {
    ...
    await update_file(`out/www/${page}.html`, html_ugly(Page(page, html)));
  }
  ...
  for (const [from, to] of redirects) {
    await update_file(`out/www/${from}`, html_ugly(Redirect({ path: to })));
  }
