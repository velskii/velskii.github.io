...
async function spell_check_with_llm(
  ...
  const prompt = `You are a professional editor. Please identify typos and grammatical errors in the following blog post.
...
${content}`;
  ...
  if (replacements_match) {
    const replacement_lines = replacements_match[1]
      .split("\n")
